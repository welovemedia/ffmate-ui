...
if (editPresetId) {
  await useFFMate()
    ...
    .catch((e: unknown) => {
      useDebug("presets")("failed to load global presets", e);
    });
}
  ...
  const n = {
    ...
    priority:
      typeof form.priority === "string"
        ...
        : form.priority,
    retries:
      typeof form.retries === "string" ? parseInt(form.retries) : form.retries,
...
<template>
  <div class="grid grid-cols-1 gap-x-8 gap-y-8 pt-10 md:grid-cols-3">
    ...
    <form class="md:col-span-2 rounded-lg bg-white/2" @submit.prevent="save">
      <div class="px-4 py-6 sm:p-8">
        <div class="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
          ...
          <div class="sm:col-span-full">
            ...
            <Button class="w-full mt-4" @click.stop="form.labels.push('')"
              >+ Label</Button
            >
          </div>
                    ...
                    <FormFieldSelect
                      ...
                      :options="
                        Object.values(WebhookEvent)
                          .filter(
                            (w) =>
                              w.startsWith('preset.') || w.startsWith('task.'),
                          )
                          .map((e) => ({
                            label: e,
                            key: e,
