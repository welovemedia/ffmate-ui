...
<template>
  <div class="pt-12">
    <div class="px-4 sm:px-0">
      ...
      >
        <FolderIcon v-if="task.source === 'watchfolder'" class="size-3" />
    ...
    <div class="mt-6 border-t border-white/10">
      <dl class="divide-y divide-white/10">
        ...
        <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                ...
                <div
                  ...
                  :class="{
                    'bg-blue-500/50':
                      ...
                      task.status === 'PRE_PROCESSING' ||
                      task.status === 'POST_PROCESSING',
                    'bg-green-400/50': task.status === 'DONE_SUCCESSFUL',
                    'bg-red-400/50': task.status === 'DONE_ERROR',
                    'bg-yellow-400/50': task.status === 'DONE_CANCELED',
                  }"
          ...
          <template v-if="task.startedAt && !task.finishedAt">
            ...
            <dd class="mt-1 text-sm/6 text-gray-400 sm:col-span-2 sm:mt-0">
              ...
              <span v-if="task.status === 'RUNNING' && task.lastRemaining"
                >|
                {{
          ...
          <dd class="mt-1 text-sm/6 text-gray-400 sm:col-span-2 sm:mt-0">
            ...
            >
              ...
              <dd class="mt-1 text-sm/6 text-gray-400 sm:col-span-2 sm:mt-0">
                finished
                {{
                  useTimeAgo(new Date(task.preProcessing.finishedAt), {
                    showSecond: true,
                    updateInterval: 1_000,
                  })
                }}
          ...
          <dd class="mt-1 text-sm/6 text-gray-400 sm:col-span-2 sm:mt-0">
            ...
            >
              ...
              <dd class="mt-1 text-sm/6 text-gray-400 sm:col-span-2 sm:mt-0">
                finished
                {{
                  useTimeAgo(new Date(task.postProcessing.finishedAt), {
                    showSecond: true,
                    updateInterval: 1_000,
                  })
                }}
        ...
        >
          ...
          <template v-for="(webhook, index) in task.webhooks" :key="index">
        ...
        >
          ...
          <dt class="text-sm/6 font-medium text-white ml-2">Details</dt>
          <dd class="mt-1 text-sm/6 text-gray-400 sm:col-span-2 sm:mt-0">
            Version: {{ task.client?.version ?? "n/a" }} | OS:
            {{ task.client?.os ?? "n/a" }} | Arch:
            {{ task.client?.arch ?? "n/a" }}
          </dd>
          <dt class="text-sm/6 font-medium text-white ml-2">Labels</dt>
          <dd class="mt-1 text-sm/6 text-gray-400 sm:col-span-2 sm:mt-0">
            {{ task.client?.labels?.join(", ") ?? "n/a" }}
          </dd>
        </div>
      </dl>
    </div>
