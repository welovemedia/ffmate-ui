...
const originalStore = defineStore("watchfolder", {
    state: (): { watchfolder: Watchfolder[]; total: number } => {
        return { watchfolder: [], total: 0 };
    },
    actions: {
            ...
            for (const data of tmpUpdates) {
                switch (data.subject) {
                    case "watchfolder:created":
                        store.value!.watchfolder.unshift(
                            data.payload as Watchfolder,
                        );
                        break;
                    case "watchfolder:updated":
                        store.value!.watchfolder = store.value!.watchfolder.map(
                            (t) => {
                                if (
                                    t.uuid ===
                                    (data.payload as Watchfolder).uuid
                                ) {
                    ...
                    case "watchfolder:deleted":
                        store.value!.watchfolder =
                            store.value!.watchfolder.filter(
                                (t) =>
                                    t.uuid !==
                                    (data.payload as Watchfolder).uuid,
                            );
